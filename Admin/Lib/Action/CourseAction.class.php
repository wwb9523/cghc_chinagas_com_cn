...
class CourseAction extends BaseAction
{
    ...
    function _initialize()
    {
        parent::_initialize();
        //取所有分类，过滤出新闻模块主ID
        $category = D('Category')->where('module="Course"')->Order('display_order DESC,id DESC')->findAll();
        foreach ($category as $item)
        {
            if($item['parent_id']==0)$this->parentId=$item['id'];
        }
        //取主ID下属分类
    ...
    public function index()
    {
        ...
        $condition['b.module']=array('in','Course');
        $count=$this->dao->Table(C('DB_PREFIX').'course a')->Join(C('DB_PREFIX').'category b on a.category_id=b.id')->where($condition)->count();
        $listRows = empty($pageSize) || $pageSize > 100 ? 15 : $pageSize ;
        $p = new page($count, $listRows);
        $dataList = $this->dao->Table(C('DB_PREFIX').'course a')->Join(C('DB_PREFIX').'category b on a.category_id=b.id')->where($condition)->Field('a.*,b.title as category')->Order($setOrder)->Limit($p->firstRow.','.$p->listRows)->findAll();
        $page = $p->show();
        if ($dataList !== false)
        {
    ...
    {
        parent::_checkPermission('Course_insert');
